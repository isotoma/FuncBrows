...
class FuncTests(unittest.TestCase):
    ...
    def _test_form_catchall(self, f):
        f.open('/')
        f.form_name = '*'
        f.set_form_text_field('q', 'test')
        f.submit_form()
    ...
    @run_in_thread
    def test_form_catchall_testbrowser(self):
        f = FuncBrows('testbrowser', 'http://localhost:%s' % self.portno)
        self._test_form_catchall(f)

    @run_in_thread
    def test_form_catchall_selenium(self):
        f = FuncBrows('*firefox3', 'http://localhost:%s' % self.portno, host = '127.0.0.1', port = 4444)
        self._test_form_catchall(f)

    def _test_submit_form_by_identifier(self, f):
        f.open('/')
        f.form_name = '*'
        f.set_form_text_field('q', 'test')
        f.submit_form(identifier = "SubmitButton")
        self.assertTrue('test' in f.page_contents)

    @run_in_thread
    def test_submit_form_by_identifier_testbrowser(self):
        f = FuncBrows('testbrowser', 'http://localhost:%s' % self.portno)
        self._test_submit_form_by_identifier(f)

    @run_in_thread
    def test_submit_form_by_identifier_selenium(self):
        f = FuncBrows('*firefox3', 'http://localhost:%s' % self.portno, host = '127.0.0.1', port = 4444)
        self._test_submit_form_by_identifier(f)
